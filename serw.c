       ...
       if (sock < 0)
         {
           ...
           /*fprintf (stderr, "***ERROR*** mainserver_make_socket/socket: family=%d  type=%d protocol=%d ret=%d errno=%d\n",
       ...
       if (bind (sock, (struct sockaddr *) &name, sizeof (name)) < 0)
         {
           ...
           /*fprintf (stderr, "***ERROR*** mainserver_make_socket/bind: socket=%d  localaddr=%p addrlen=%d errno=%d\n",
                          sock, (struct sockaddr *)&name, sizeof(name), errno);*/ 
           return -2;
         }
       return sock;
#else
       ...
       if (nbytes < messagesize)
         {
           /* Write error */
           /*perror ("write");*/
           /*fprintf (stderr, "***ERROR*** mainserver_write_to_client: socket=%d  msg=%s msglen=%d flags=%d errno=%d\n",
                          filedes, message, messagesize, 0, errno);*/ 
           return -1;
         }
       else
         return nbytes;
       ...
       if (nbytes < 0)
         {
           ...
           /*(fprintf (stderr, "***ERROR*** mainserver_read_from_client: socket=%d  buffer=%s length=%d flags=%d errno=%d\n",
                          filedes, buffer, buffersize-1, 0, errno);*/   
           return -2;
         }
       else
         {
       ...
       if (listen (sock, maxconnqueued) < 0)   /* <== at most 1 connection queued */
         {
           ...
           /*fprintf (stderr, "***ERROR*** mainserver/listen: socket=%d  queuelen=%d errno=%d\n",
                          sock, maxconnqueued, errno);*/
           close(sock); 
           return -1;
         }
           ...
           if (select (FD_SETSIZE, &read_fd_set, NULL, NULL, NULL) < 0)
             {
               ...
               /*fprintf (stderr, "***ERROR*** mainserver/select: numfds=%d  refds=%p wrfds=%p exfds=%p time=%p errno=%d\n",
                          FD_SETSIZE, &read_fd_set, NULL, NULL, NULL, errno);*/
                     ...
                     if (new < 0)
                       {
                         ...
                         /*fprintf (stderr, "***ERROR*** mainserver/accept: socket=%d  addr=%p addrlen=%p errno=%d\n",
                                                     sock, (struct sockaddr *) &clientname, &size, errno);*/
                         close(sock);
                         return -3;
                       }
#if GEN_LOGS
