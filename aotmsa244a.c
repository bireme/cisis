...
32701  "m90.mdl/mdltwux^m2"
	    ...
	    char *fldp=FIELDP(0),*p;
            LONGX xmfn;
//prtfields(recp,0);
            if (DIRtag(0) != 32701) fatal("aotmsa2/TA/mdlproc/tag");
            *(fldp+DIRlen(0))='\0';
            p=strchr(fldp,'^');
...
//prtfields(recp,0);
            if (mdlgizmo) {
                if (fldupdat(irec,"GTAB130")) fatal("aotmsa2/TA/mdlproc/proc2");
    ...
    *tabfreqFp=tabfreqF;
    ...
    for (mfn=0L, v351=1; mfn<ntabs; v351++) {
        ...
        if (mydrec) { /* use decs/v20 (mydecs/v22) as 2nd sort key for sortF */
		    ...
		    printf("+++ v351=%"_LD_"\n",(LONGX)v351);
		    fatal("aotmsa2/rutfreqF/mydecs/TAG022");
	    }
