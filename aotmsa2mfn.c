...
*/          /* VRECdbxp(boolridx)=RECdbxp; */ /* b7_exp faz "RECdbxp = dbxstorp(qrydbnp)" */
        ...
        */
        ...
        if (p || errno) printf("Expression syntax error %d: '%s'\n",errno,p);
            ...
            */
            if (deffmhxp) {
                char *batchp=deffbuffup;
                size_t left=sizeof(deffbuffup);
                int xdir;
                ...
                if ((xdir=fieldx(boolridx,MFQTOBJ0,1)) >= 0) {
                    ...
                    for (b7p=(unsigned char *)FIELDP(xdir); *b7p; ) {
                        ...
                        if (*b7p != ZLDV) {
                            *batchp++ = *b7p++; /* move other */ left--;
                            continue;
                        }
                        *batchp++ = *b7p++; /* move b7opr */ left--;
                        *batchp++ = *b7p++; /* move postlen */ left--;
                        v351=atol(b7p);
                        ...
                        if (v351) {
                            ...
                            while (*batchp) { *batchp=isisuctab[*batchp]; batchp++; left--; }
                        }
                        *batchp++ = *b7p++; /* move OPRNULL */ left--;
                    } /* end for (b7p) */
