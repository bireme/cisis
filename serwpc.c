       ...
       */
       for (i=1; i < argc; i++) {
            ...
            if (strncmp(p,"cipar=",6) == 0) {
                cipar=p+6; //dbxcdcip=loadfile(NULL,'\0',p+6,NULL,0L,'\n');
                continue;
            }
            ...
            if (p) {
                ...
                if (!dbxcdcip) dbxcdcip=loadfile(NULL,'\0',cipar,NULL,0L,'\n'); // env
                if (!awtfp) awtfp=wtfnew((argc>1)?atoi(argv[1]):0,dbxcdcip);    // new

                for (p=argv[i]; *p; p++) if (*p=='|') *p='\n'; // test text=..|..

                rc = loadcoll(cmd, awtfp, argv[i], buffer);
                if (rc < 0) { 
