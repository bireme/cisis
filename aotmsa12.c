...
#define FORCECGI 0
    ...
    putenv("QUERY_STRING=lang=p&action=C%2CTB%2CDB&amdl=amdl351&bool=009068&bool=034268&bool=034269&loop=100&sort=f&from=1&count=20&x=39&y=3");
#endif
      ...
      for (dirp=MFRdir, xdir=iocc=0, loop=MFRnvf; loop--; dirp++, xdir++) {
        if (dirp->tag == cgitag) {
          ...
          */
          ...
          if (strncmp(p,"^nloop^v"   , 4+4) == 0) { prmloopTA =recsfldv(NULL,p,plen,""); parmloopTB =atol(prmloopTA ); }
          if (strncmp(p,"^nfrom^v"   , 4+4) == 0) { prmfromDB =recsfldv(NULL,p,plen,""); parmfromDB =atol(prmfromDB ); }
          if (strncmp(p,"^ncount^v"  , 5+4) == 0) { prmcountDB=recsfldv(NULL,p,plen,""); parmcountDB=atol(prmcountDB); }
    ...
    */
    ...
    if (!langp)       langp       = "p";
    if (!parmloopTB)  parmloopTB  = 1;
    if (!parmtellTA)  parmtellTA  = 0; /* */
    if (!parmfromDB)  parmfromDB  = 1;
    if (!parmcountDB) parmcountDB = 0;
    ...
    */
    for (; argnext < argc; argnext++) {
        ...
        if (strncmp(p,"loop?",5) == 0 || strncmp(p,"loop=",5) == 0) {
            if (sscanf(p+5,"%"_LD_,&parmloopTB) != 1) fatal(p);
            continue;
        }
        if (strncmp(p,"from?",5) == 0 || strncmp(p,"from=",5) == 0) {
            if (sscanf(p+5,"%"_LD_,&parmfromDB) != 1) fatal(p);
            continue;
        }
        if (strncmp(p,"count?",6) == 0 || strncmp(p,"count=",6) == 0) {
            if (sscanf(p+6,"%"_LD_,&parmcountDB) != 1) fatal(p);
            continue;
        }
    ...
    if (actionp) {
      for (q=actionp; *q; ) {
         ...
         if (*q == 'T' && strncmp(q,"TB",2) == 0)
                   {   actionTB=1;                    if (*(q+=2) == ',') q++; continue; }

         if (*q == 'D' && strncmp(q,"DB",2) == 0)
                   {   actionDB=1;                    if (*(q+=2) == ',') q++; continue; }
...
if (parmtrace) {
  printf("+++trace: tmp1dbnp=%s\n",tmp1dbnp);
  printf("+++trace: actionp=%s\n",actionp);
  printf("+++trace: actionDD=%d\n",actionDD);
  printf("+++trace: actionTB=%d\n",actionTB);
  printf("+++trace: actionDB=%d\n",actionDB);
  printf("+++trace:     sort=%s\n",parmsortp);
...
if (actionTB) {
    ...
    */
    ...
    if (parmatmdl99) {
        ...
        LONGX /*nu irec,*/itrm;
        char *p;
        TERM(itrm=ntrms,atmdldbnp,"999999"); if (TRMrc != RCNORMAL) fatal("aotmsa2/atmdl/TERM");
        RECORD(/*nu irec=*/nrecs,atmdldbnp,posting(itrm,1));  if (TRMrc != RCNORMAL) fatal("aotmsa2/atmdl/RECORD");
        p=recfield(NULL,boolridx,ATMDLTAG999,1,"");
        docsTOTamdl = atol(p); if (docsTOTamdl < 1) fatal("aotmsa2/atmdl/999999");
    }
        ...
        */
        ...
        if (p || errno) printf("Expression syntax error %d: '%s'\n",errno,p);
            ...
            p=b7_run(cib7p,boolridx,amdldbnp,buffup,asize,
                booltell,isisuctab,""/*btch0p*/,booltlir,booldbcrec,&errno);
            if (p || errno) printf("Execution error %d: '%s'\n",errno,p);
            else {
                // if (b7bufferp->hcase != MFNSTRING) fatal("aotmsa2/MFNSTRING");
...
#define MAXN05 30
    ...
    for (boolh=1; boolh <= boolhits; boolh+=parmloopTB) {
        ...
        if (b7bufferp->hcase == BITSTRING)
            ...
            mfn = *mfnlistp; mfnlistp+=parmloopTB;
                    ...
                    for (dirp=MFRdir, xdir=0, loop=MFRnvf; loop--; dirp++, xdir++) {
                        if (dirp->tag == TAG020) {
                            ...
                            if (*p == 'C' or *p == 'D') { /* ... */
                                ...
                                if (!found) {
                                    if (n20 >= MAXN20) fatal("aotmsa2/TA/mydecs/MAXN20");
...
ddecsdbnp=decsdbnp;
        ...
        */
        ...
        if (rutfreqW(tabfreqF,ntabs,amdldbnp,docsTOTxA,docsTOTamdl,tabcntArankF,tabcntArankW,&tabfreqW,(parmloadmdl?tabcntMDL:NULL),gtabcntA)) exit(4);
...
if (ntabs) {
    ...
    */
    ...
    if (parmloopTB ) { sprintf(batchp,"a3007|%"_LD_"|", parmloopTB  ); batchp+=strlen(batchp); }
    if (parmfromDB ) { sprintf(batchp,"a3008|%"_LD_"|", parmfromDB  ); batchp+=strlen(batchp); }
    if (parmcountDB) { sprintf(batchp,"a3009|%"_LD_"|", parmcountDB ); batchp+=strlen(batchp); }
...
/*nu     char *vMFQTAENV; */
    ...
    if (parmtrace) printf("+++trace: DB/namedbnp=%s\n",namedbnp);
...
/*nu     vMFQTAENV = recfield(NULL,boolridx,MFQTAENV,1,""); */
    ...
    if (parmsortx == 'd') parmfrom=1;
        ...
        parmfrom=parmfromDB;
        if (parmcountDB) parmto=parmfromDB+parmcountDB-1;
        if (parmto > MF0nxtmfn-1) parmto=MF0nxtmfn-1;
        count2=parmto-parmfromDB+1;
    }
...
*/
    ...
    if (vMFQTQRY0) {
        ...
        for (p=vMFQTQRY0; *p; ) {
            ...
            if (v351) {
                ...
                recfield(batchp,drec,DECSTAGP,1,"");    batchp+=strlen(batchp);
            }
    ...
    /* do it */
    for (mfn1=parmfrom/*DB*/; mfn1 <= parmto; mfn1++) {
        ...
        if (RECrc != RCNORMAL) fatal("aotmsa2/DB/RCNORMAL");
        count1++;
        if (parmtrace) printf("+++ mfnDB=%"_LD_"\n",mfn1);
...
*/
            ...
            if (!*v1f || !*v1w || strcmp(v1f,v1w)) fatal("aotmsa2/DB/join");
/*
...
*/
        ...
        */
        ...
        /* add/del fields */
        if (fldupdat(irec,fmtarea)) fatal("aotmsa2/DB/fldupdat/fmt");
        if (fldupdat(irec,envarea)) fatal("aotmsa2/DB/fldupdat/env");
        ...
        */
        ...
        for (boolh=1; ; boolh++) {
            ...
            if (boolhits) {
                ...
                if (mfn <= 0) fatal("aotmsa2/DD/decsdbnp/hit");
    
                if (parmtrace) printf("+++ hit#%"_LD_" mfn=%"_LD_"\n",boolh,mfn);
    
                RECORD(drec,decsdbnp,mfn);
                if (RECrc != RCNORMAL) fatal("aotmsa2/DD/decsdbnp/RCNORMAL");
...
//                if (fldupdat(drec,fmtarea)) fatal("aotmsa2/DD/fldupdat/x");
            }
    ...
    *tabfreqFp=tabfreqF;
    ...
    for (mfn=0L, v351=1; mfn<ntabs; v351++) {
        ...
        if (drec) { /* use decs/v20 as 2nd sort key for sortF */
            ...
            recfield(buffup,drec,TAG020,1,""); if (!*buffup) fatal("aotmsa2/rutfreqF/decs/TAG020");
    ...
    *tabfreqWp=tabfreqW;
    ...
    for (sortp=sort_tab, tabF=tabfreqF, mfn=1L; mfn<=ntabs; mfn++, tabF++, sortp++) {
        ...
        if (tabcntMDL)
            ...
            TERM(itrm,amdlname,x351); if (TRMrc != RCNORMAL) fatal("aotmsa2/rutfreqW/TERM/amdl");
