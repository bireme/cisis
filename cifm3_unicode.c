...
#endif /* CICPP */
{
    ...
    if (slen > max_sfield) {
        char *fld = FIELDP(dd);
        int slen2 = slen = max_sfield-3;

        while(slen2 > 0) {
            if (skipchar_UTF8(fld,slen2)>(fld+slen)) {
                slen2--;
            }
        }
  ...
  case left_end:
  {
   ...
   if (op1->classe!=string || !is_numeric(op2->classe)){
      ...
      if (infoi(ni)==left_end){
        ...
        if (lng <0) lng=0;
        //tt[lng]=null_char;
        skipchar_UTF8(tt,lng)[0]=null_char;
#if TRACE_STRFUN
        printf ("%s\n",tt);
#endif
      }
 ...
 case ss_end:    /*A10* ss(pos,length,string) */
  {
   ...
   if (op1->classe!=string || !is_numeric(op2->classe) ||
      ...
      xn_fim=cast_to_int(op3);
         ...
         tt=(char *) skipchar_UTF8(tt,xn_inic-1);
         //lng=strlen(tt);
         lng=strlen_UTF8(tt);
         if (xn_fim<0) xn_fim=0;
         if (xn_fim>lng) xn_fim=lng;
         //tt[xn_fim]=null_char;
         skipchar_UTF8(tt,xn_fim)[0]=null_char;
#if TRACE_STRFUN
         printf ("%s\n",tt);
#endif
        restore_context();
...
*/
   ...
   case size_end:
  {
        ...
        elem.op.l= (LONGX ) strlen_UTF8(out);
        elem.classe=long_n;
        restore_context();
        IFERR_GOTO;
        push(&elem);
