/* =============================================================================
   AOTMSA3     Adalberto Otranto Tardelli / Meide da Silva Ancao
               Application 1
               by AOT, 05/01/2002
   ...
   aotmsa3.c   CI/tabula casos segundo variavel/variaveis de controle
   -----------------------------------------------------------------------------

   aotmsa3 <action>[,...] [<option> [...]]
...
*/
    ...
    */
    ...
    /*fmt_fsiz=maxmfrl;*/
    record(irec0=nrecs,"null",0L); if (irec0) fatal("aotmsa3/irec0");
    term(itrm0=ntrms,"null","!");  if (itrm0) fatal("aotmsa3/itrm0");
...
PATH_CGI-BIN=/users/www/cgi-bin/iah/
...
#if UNIX
    if (!cipfnamp)    cipfnamp    = "/home/aot/bases/cip.cip";
    if (!decsdbnp)    decsdbnp    = "mydecs";
    if (!amdldbnp)    amdldbnp    = "amdl351";
#else
    if (!cipfnamp)    cipfnamp    = "atmap/cip.cip";
    if (!decsdbnp)    decsdbnp    = "mydecs";
    if (!amdldbnp)    amdldbnp    = "amdl351";
#endif
    if (!decs0dbnp)   decs0dbnp   = "decs";
    if (!tmp1dbnp)    tmp1dbnp    = NULL;
    if (!pftfnamp)    pftfnamp    = "aotmsa3A.pft";
    if (!langp)       langp       = "p";
    ...
    if (argc < 2 && !wwwp) {
        printf("%s",cicopyr("Application AOTMSA3"));
        printf(" \n");
        printf("aotmsa3 <action>[,...] [<option> [...]] \n");
        printf(" \n");
...
*/
    ...
    if (namedbnp) {
        ...
        if (!dbxtmpnm("CI_TEMPDIR", 0, namedbnarea)) fatal("aotmsa3/dbxtmpnm");
...
#define LILPROC 1
#if LILPROC
    FMT_CODE *xlilfmtp=NULL;
#define FMTASIZE 8000
...
#define MDLPROC 0
    ...
    p=dbxcipar(NULL,"TAB130.*",'=');
    if (dbxcipok) mdlgizmo=1;
#endif /* MDLPROC */
    ...
    if (boolqryp) { /* boolqryp_new */
        ...
        for (p=boolqryp; *p; ) {
            ...
            if (v351) {
                ...
                if (RECrc != RCNORMAL) fatal("aotmsa3/boolqryp/RECrc/RCNORMAL");
                if (parmqdollar) {
    ...
    if (boolqryp) { /* boolqryp_3213 */
        ...
        for (p=boolqryp; *p; ) {
            ...
            if (v351) {
                if (!mydrec) mydrec=nrecs;
                RECORD(mydrec,"mydecs",v351);
                if (RECrc != RCNORMAL) fatal("aotmsa3/boolqryp_3213/RECrc/RCNORMAL");
...
*/          /* VRECdbxp(boolridx)=RECdbxp; */ /* b7_exp faz "RECdbxp = dbxstorp(qrydbnp)" */
        ...
        */
        ...
        if (p || errno) printf("Expression syntax error %d: '%s'\n",errno,p);
            ...
            if (fldupdat(boolridx,buffup) != NULL) fatal("aotmsa3/fldupdat/boolridx");
            ...
            if (deffmhxp) {
                ...
                if (fldupdat(booltlir,buffup)) fatal("aotmsa3/booltlir/fldupdat");
            }
...
#define MAXN05 30
    ...
    for (boolh=1; boolh <= boolhits; boolh+=parmloopTA) {
        ...
        if (mfn <= 0) fatal("aotmsa3/TA/amdldbnp/hit");

        if (parmtrace) printf("+++ hit#%"_LD_" mfn=%"_LD_"\n",boolh,mfn);

        RECORD(irec,amdldbnp,mfn);
        if (RECrc != RCNORMAL) fatal("aotmsa3/TA/amdldbnp/RCNORMAL");

#if MDLPROC
#if 0
[aot@serverofi test]$ /utl/lind/mx cipar=../bases/cip.cip amdl351
mfn=     1
32701  "m90.mdl/mdltwux^m1"
mfn=     2
32701  "m90.mdl/mdltwux^m2"
#endif
        if (mdlproc) {
#if 0
            ...
            LONGX xmfn;
//prtfields(recp,0);
            if (DIRtag(0) != 32701) fatal("aotmsa3/TA/mdlproc/tag");
            *(fldp+DIRlen(0))='\0';
            p=strchr(fldp,'^');
            if (!p) fatal("aotmsa3/TA/mdlproc/mfn");
            xmfn=atol(p+2);
            if (xmfn < 1) fatal("aotmsa3/TA/mdlproc/xmfn");
            p=fldp; *p='M'; p[3]='\0';
            //*p='\0';
//printf("RECORD %s,%"_LD_"\n",fldp,xmfn);
            RECORD(irec,fldp,xmfn);
            if (RECrc != RCNORMAL) fatal("aotmsa3/TA/mdlproc/RCNORMAL");
//printf("RECORD %"_LD_" ok\n",xmfn);
            if (fldupdat(irec,"d370d999")) fatal("aotmsa3/TA/mdlproc/proc1");
//prtfields(recp,0);
            if (mdlgizmo) {
                if (fldupdat(irec,"GTAB130")) fatal("aotmsa3/TA/mdlproc/proc2");
//printf("gizmo %"_LD_" ok\n",xmfn);
	    }
//prtfields(recp,0);
            if (fldupdat(irec,"Gsplit=351=;")) fatal("aotmsa3/TA/mdlproc/proc3");
            MFRmfn=mfn;
//prtfields(recp,0);
//getchar();
#else
...
#if LILPROC
        if (!xlilfmtp) {
            if (fmt_gener(&xlilfmtp,",@lilacs.proc351.pft,")) fatal("aotmsa3/TA/lilproc/generpft3");
        }
        if (xlilfmtp) {
            /* apply display format */
            if (fmt_inter(xlilfmtp,irec,fmtasize,fmtarea,fmtasize) < 0) fatal("aotmsa3/TA/lilproc/pft3");
            /* apply fldupdat */
            if (fldupdat(irec,fmtarea)) fatal("aotmsa3/TA/lilproc/proc3");
        }
#endif /* LILPROC */
        ...
        for (dirp=MFRdir, xdir=0, loop=MFRnvf; loop--; dirp++, xdir++) {
            if (dirp->tag == TAG351) {
                ...
                if (1/*parmgroup*/)  {
                    ...
                    RECORD(mydrec,"mydecs",v351);
                    if (RECrc != RCNORMAL) fatal("aotmsa3/TA/mydecs/RCNORMAL");
                    ...
                    for (dirp=MFRdir, xdir=0, loop=MFRnvf; loop--; dirp++, xdir++) {
                        if (dirp->tag == TAG005) {
                           ...
                           if (n05 >= MAXN05) fatal("aotmsa3/TA/mydecs/MAXN05");
                           v05[n05++]=mfn;
                        } /* end tag == TAG005 */
                    } /* end for dirp */
                    ...
                    for (i=0; i<n05; i++) {
                        ...
                        if (!found) {
                            if (nvgs >= MAXNVG) fatal("aotmsa3/TA/mydecs/MAXNVG");
                            vgmfns[nvgs++]=mfn;
                        }
                    } /* end for v05[] */
                ...
                if (n05) { /* 18/03/2003 */
			if (v351 >= CLASSES) fatal("aotmsa3/CLASSES overflow/v351");
        ...
        */
        for (ivg=0; ivg < nvgs; ivg++) {
            ...
            if (gmfn >= CLASSES) fatal("aotmsa3/CLASSES overflow/gmfn");
...
/**/
        ...
        if (fldupdat(boolridx,envarea)) fatal("aotmsa3/TA/env/fldupdat");
        if (recupdat(booldbcrec,boolridx)) fatal("aotmsa3/TA/env/fldupdat");
    }

    /* done
    */
    if (parmtellTA)
        fprintf(stderr,"aotmsa3: %"_LD_" recs in | %"_LD_" tabs | %d recs out\n",count1,count2,ntabs);
...
*/
    ...
    for (mfn1=parmfrom/*DA*/; mfn1 <= parmto; mfn1++) {
        ...
        if (RECrc != RCNORMAL) fatal("aotmsa3/DA/RCNORMAL");
        count1++;
...
*/
            ...
            if (!*v1f || !*v1w || strcmp(v1f,v1w)) fatal("aotmsa3/DA/join");
/*
...
*/
        ...
        */
        ...
        if (RECrc != RCNORMAL) fatal("aotmsa3/DA/mydecs/RCNORMAL");
        ...
        /* add/del fields */
        if (fldupdat(irec,fmtarea)) fatal("aotmsa3/DA/fldupdat/fmt");
        if (fldupdat(irec,envarea)) fatal("aotmsa3/DA/fldupdat/env");
    ...
    if (boolqryp) { /* boolqryp */
        ...
        */
        ...
        if (p1 || errno1) ; /* printf("Expression syntax error %d: '%s'\n",errno1,p1); */
            ...
            if (booltmpnam[0]) hitfnamp=booltmpnam;
            sprintf(buffup,"D%dA%d|%s|",MFQTHMEM,MFQTHMEM,hitfnamp);
            if (fldupdat(boolridx,buffup) != NULL) fatal("aotmsa3/DD/fldupdat/boolridx");
        ...
        */
        ...
        for (boolh=1; ; boolh++) {
            ...
            if (boolhits) {
                ...
                if (mfn <= 0) fatal("aotmsa3/DD/dexdbnp/hit");

                if (parmtrace) printf("+++ hit#%"_LD_" mfn=%"_LD_"\n",boolh,mfn);

                RECORD(drec,dexdbnp,mfn);
                if (RECrc != RCNORMAL) fatal("aotmsa3/DD/dexdbnp/RCNORMAL");
            }
            else {
...
*/
        ...
        for (sortp=sortareap, boolh=1; nsort-- ; boolh++, sortp++) {
            ...
            RECORD(drec,dexdbnp,mfn);
            if (RECrc != RCNORMAL) if (boolh > 1) fatal("aotmsa3/DD/dexdbnp/sorted/RCNORMAL");
            if (RECtype != TYPEMFR) fatal("aotmsa3/DD/dexdbnp/sorted/TYPEMFR");
#endif /* SORTDECS */
            ...
            */
            ...
            if (fldupdat(drec,fmtarea)) fatal("aotmsa3/DD/fldupdat/fmt");
            if (fldupdat(drec,envarea)) fatal("aotmsa3/DD/fldupdat/env");
...
typedef struct outfreqF_sort_entry {
    ...
    */
    ...
    if (nbytes >= (LONGX)ALLOMAXV) fatal("aotmsa3/rutfreqF/tabfreqF/ALLOMAXV");
    p=(char *)ALLOC((ALLOPARM)(nbytes));
    if (p == ALLONULL) fatal("aotmsa3/rutfreqF/tabfreqF/ALLOC");
    memset(p,0,nbytes); /* init */
    tabfreqF=(TABFREQF_ENTRY *)p;
    *tabfreqFp=tabfreqF;

    nbytes=sizeof(OUTFREQF_SORT_ENTRY)*ntabs+sizeof(OUTFREQF_SORT_ENTRY);
    if (nbytes >= (LONGX)ALLOMAXV) fatal("aotmsa3/rutfreqF/sort_tab/ALLOMAXV");
    p=(char *)ALLOC((ALLOPARM)(nbytes));
    if (p == ALLONULL) fatal("aotmsa3/rutfreqF/sort_tab/ALLOC");
    ...
    for (mfn=0L, v351=1; mfn<ntabs; v351++) {
        ...
        if (mydrec) { /* use decs/v20 (mydecs/v22) as 2nd sort key for sortF */
            ...
            recfield(buffup,mydrec,MYDECSTAG022,1,""); if (!*buffup) {
		    printf("+++ v351=%"_LD_"\n",(LONGX)v351);
		    fatal("aotmsa3/rutfreqF/mydecs/TAG022");
	    }
...
//    char pesoTTxAout[4]; /* v2221[1] = 'out' se peso < 1 */
    ...
    */
    ...
    if (nbytes >= (LONGX)ALLOMAXV) fatal("aotmsa3/rutfreqW/tabfreqW/ALLOMAXV");
    p=(char *)ALLOC((ALLOPARM)(nbytes));
    if (p == ALLONULL) fatal("aotmsa3/rutfreqW/tabfreqW/ALLOC");
    memset(p,0,nbytes); /* init */
    tabfreqW=(TABFREQW_ENTRY *)p;
    *tabfreqWp=tabfreqW;

    nbytes=sizeof(OUTFREQW_SORT_ENTRY)*ntabs+sizeof(OUTFREQW_SORT_ENTRY);
    if (nbytes >= (LONGX)ALLOMAXV) fatal("aotmsa3/rutfreqW/sort_tab/ALLOMAXV");
    p=(char *)ALLOC((ALLOPARM)(nbytes));
    if (p == ALLONULL) fatal("aotmsa3/rutfreqW/sort_tab/ALLOC");
    sort_tab=(OUTFREQW_SORT_ENTRY *)p;
    sort_tmp=sort_tab+ntabs;


    if (!tabcntMDL) fatal("aotmsa3/rutfreqW/tabcntMDL");
...
//    LONGX delta;
    ...
    */
    ...
    if (nbytes >= (LONGX)ALLOMAXV) fatal("aotmsa3/rutfreqD/tabfreqD/ALLOMAXV");
    p=(char *)ALLOC((ALLOPARM)(nbytes));
    if (p == ALLONULL) fatal("aotmsa3/rutfreqD/tabfreqD/ALLOC");
    memset(p,0,nbytes); /* init */
    tabfreqD=(TABFREQD_ENTRY *)p;
    *tabfreqDp=tabfreqD;

    nbytes=sizeof(OUTFREQD_SORT_ENTRY)*ntabs+sizeof(OUTFREQD_SORT_ENTRY);
    if (nbytes >= (LONGX)ALLOMAXV) fatal("aotmsa3/rutfreqD/sort_tab/ALLOMAXV");
    p=(char *)ALLOC((ALLOPARM)(nbytes));
    if (p == ALLONULL) fatal("aotmsa3/rutfreqD/sort_tab/ALLOC");
          ...
          if (1) {
              ...
              for (dirp=MFRdir, xdir=0; xdir < MFRnvf; dirp++, xdir++)
                if (dirp->tag == TAG005) {
                    ...
                    p=FIELDP(xdir);
                    for (loop=dirp->len; loop--; p++) mfn=mfn*10+((int)(*p)-(int)'0');
                    if (mfn > CLASSES) fatal("aotmsa3/rutfreqD/TAG005/CLASSES");
